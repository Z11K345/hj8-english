...
LOG = os.path.join(BASE, '_dbg119b.txt')
out = []
...
out.append('total=%d' % len(items))
for i,it in enumerate(items):
    out.append('%d: %s' % (i+1, it.replace('\n',' ')))
# also count raw 【答案】 and numbered in tail (after jiexi removal)
body = _remove_jiexi(tail)
out.append('--- raw numbered count after jiexi: %d' % len(re.findall(r'(?m)^\s*\d{1,3}\s*[.．.]\s', body)))
