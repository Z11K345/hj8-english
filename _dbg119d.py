...
LOG = os.path.join(BASE, '_dbg119d.txt')
out = []
...
# find all line-start numbers
nums = [int(m.group(1)) for m in re.finditer(r'(?m)^\s*(\d{1,3})\s*[.．.]', body)]
present = set(nums)
missing = [n for n in range(1,120) if n not in present]
out.append('line-start nums count=%d distinct=%d' % (len(nums), len(present)))
out.append('missing 1-119: %s' % missing)
# show context around 117-119 in clean body
idx = body.find('117')
out.append('around 117: ' + body[idx:idx+120].replace('\n','\\n'))
