...
- 系统提示词限定“只围绕沪教八上英语答疑、不编造、不跑题、一切为了考试”。
"""
import os
...
chat_js = r"""
const SYSTEM_PROMPT = '你是沪教·英语 八年级上册（全国版·2025版）的辅导老师，面向初二学生。规则：\n'
 + '1) 只围绕这本教材答疑（词汇、语法、阅读、写作、听力、发音、单元主题），不跑题、不引入教材外话题；\n'
 + '2) 不编造教材没有的内容；涉及教材外知识时明确标注“需核实”；\n'
 + '3) 解释通俗、适合初中生，多用例子；\n'
 + '4) 一切为了考试：重点讲时态、重点单词/短语、句型结构、考点；\n'
 + '5) 可用中文回答，必要时穿插英文例句。';
...
async function send(){
  ...
  try{
    ...
    bot.textContent = friendlyError(e);
  }finally{
    generating=false; sendBtn.disabled=false; controller=null; body.scrollTop=body.scrollHeight;
  }
}
...
<div class="ai-mask" id="aiMask">
  <div class="ai-panel">
    ...
    <div class="ai-body" id="chatBody" style="display:none;">
      <div class="msg sys">我是这本教材的辅导老师，有疑问就在下方聊天框输入，或粘贴/上传题目截图，我来为你解答。</div>
    </div>
    ...
    <div class="ai-input" id="aiInput" style="display:none;">
      <textarea id="chatInput" placeholder="就课文提问，或粘贴/上传题目图片"></textarea>
